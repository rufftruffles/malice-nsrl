...
The engine looks a file's SHA1 (or MD5) up in the NIST **RDSv3** (Reference
Data Set v3) SQLite database and records the result in Elasticsearch under
...
- **Exact lookup, not a Bloom filter.** The old plugin built a probabilistic
  Bloom filter from the RDS 2.XX CSV (`NSRLFile.txt`) with a 0.1%
  false-positive rate. This engine performs an exact `SELECT ... FROM FILE
  WHERE sha1 = ?` against the authoritative RDSv3 **SQLite** database.
...
## The database (D12) and the disk-constrained build

Project decision D12 calls for **RDSv3 2026.03.1 `modern_minimal`**. That set
is **18 GB** (`RDS_2026.03.1_modern_minimal.zip`), which does not fit the build
host (13 GB free). Per the task's "pick the smallest usable variant" guidance,
this image bundles the **NIST RDSv3 curated demonstration set**
(`RDS_2021.12.2_curated`, 292 MB `.db`, 677,723 files / 408,811 distinct
SHA-256) — the smallest real RDSv3 SQLite DB — fetched and SHA-1-verified at
build time.
...
```
docker build --build-context pkgs=../malice-plugins -t malice/nsrl:latest .
```
